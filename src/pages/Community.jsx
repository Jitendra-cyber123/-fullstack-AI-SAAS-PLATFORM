    ...
    <div className='flex-1 h-full flex flex-col gap-4 p-6'>
        ...
        <div className='bg-white h-full w-full rounded-xl overflow-y-scroll'>
          {creations.map((creations,index)=>(
            ...
            group-hover:bg-gradient-to-b from-transparent to-block/80 text-white
            rounded-lg'>
              
              <p className='text-sm hidden group-hover:block'> {creations.prompt}</p>
              <div className='flex gap-1 items-center'>
                <p>{creations.likes.length}</p>
                <Heart  onClick={()=> imageLikeToggle(creations.id)}
                  className={`min-w-5 h-5 hover:scale-110
                  cursor-pointer ${creations.likes.includes(user.id) ? 'fill-red-500 text-red-600' : 'text-white'}`}/>
